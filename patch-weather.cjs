...
const service = `type WeatherResult = {
  text: string;
};
...
fs.writeFileSync('src/services/weather.ts', service);

const file = 'src/verticals/SportVertical.tsx';
let c = fs.readFileSync(file, 'utf8');

if (!c.includes('getEventWeather')) {
  c = c.replace(/import .* from ["']react["'];?\n/, m => m + 'import { getEventWeather } from "../services/weather";\n');
}

if (!c.includes('const [weatherText')) {
  c = c.replace(
